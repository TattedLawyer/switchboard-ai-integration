...
| `PENDING_PROPOSAL_CAP` | `100`. Flood control: the door counts pending proposals for the tenant and refuses at the cap with a loud 429. A compromised agent host holds the door's token and can forge well-formed proposals at volume; the terminal state of that is an approval queue no human can triage, which *disables* the "nothing acts without an identified approver" constraint rather than merely annoying it. Range-checked (integer ≥ 1) at boot, naming the variable — raise it only if a human can actually keep up with the higher number | approval service door |
