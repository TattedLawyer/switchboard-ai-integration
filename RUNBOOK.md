...
| `PENDING_PROPOSAL_CAP` | `100`. Flood control: the door counts pending proposals for the tenant and refuses at the cap with a loud 429. A compromised agent host holds the door's token and can forge well-formed proposals at volume; the terminal state of that is an approval queue no human can triage, which *disables* the "nothing acts without an identified approver" constraint rather than merely annoying it. Range-checked (integer ≥ 1) at boot, naming the variable — raise it only if a human can actually keep up with the higher number. **Until A0b ships the approve/reject page there is no way to drain this queue**: `switchboard_approval` holds no `UPDATE`, so pending rows cannot reach a terminal state and the door 429s permanently once the cap is hit, legitimate proposals included. The interim remedy is manual SQL as the migration owner — `update approval.proposals set state = 'rejected' where state = 'pending'`. Recorded in KNOWN-ISSUES with A0b as the owner | approval service door |
