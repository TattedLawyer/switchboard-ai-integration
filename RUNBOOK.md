...
| `LEDGER_PATH_HUBCRM` | **governs nothing today** (gate-H I9). `hubcrm` is a `hub-hydrate` connector, whose reconcile truth is the vendor's object store, not a file — so for this variable neither half of the rule above holds: unset fails nothing, and `skip` opts nothing out. The mock genuinely writes the F-1c emission-side ledger it names (every event the store emits, chained, drops included), but `verifyLedgerChain` is called from `ledger-feed.ts` and nowhere else, so that ledger is written and verified by no shipped surface. Set it if you want the file; nothing reads it | the hubcrm MOCK's writer only |
| `INGEST_SOURCES` | `billing,support` (F-1c: `crm` left the default — its mock is retired and nothing serves port 4001). `sheets`, `stripefeed`, `hubcrm` and `casebus` are registered but **opt-in**; set them explicitly for anything real — in particular `hubcrm` MUST be enabled on the service that should run its hydration pump, or webhooks land with no snapshots and the warehouse CRM arm stays empty. Every script pins this variable explicitly. **Strict since PRE-3:** an unknown token, a blank entry, or an empty value is a boot refusal naming the variable, echoing the value and listing the accepted sources — unset the variable to ask for the default. It was previously filtered silently, so one transposed letter ran a smaller pipeline and an empty value ran none | which sources ingest polls/reconciles (scripts pin it explicitly) |
...
| `SWITCHBOARD_TENANT_ID` | unset → the default tenant `00000000-…-0000`. The ONE tenant this deployment writes under, resolved once at boot; a non-uuid refuses boot naming the variable. It governs **both halves of ingestion** — the push doors, queue envelope, worker and replay paths, AND the pull half: the scheduled backfill/hydration loop, the `backfill` CLI, every connector, and the default scope of the `reconcile`/`quarantine`/`gap-ack`/`hydrate-rearm` CLIs (an explicit `--tenant` still overrides on the three that take one). Setting it on only one half is the defect `ingest/test/pull-tenant.test.ts` exists to prevent: the hydration pump would scan an empty lane forever and backfill recovery would write shadow rows. One deployment serves one configured tenant — this is not a tenant selector and nothing routes on it | ingest doors, queue envelope, worker, DLQ replay, backfill, hydration pump, connectors, operator CLIs |
...
| `AGENT_PROPOSAL_TOKEN` | **required — fails closed** (same rule as the webhook secrets; `ALLOW_DEV_SECRETS=1` opts into the published `demo-proposal-token`). The bearer secret the agent presents at the approval service's proposal door. Note what it is not: it is not a database credential. It authenticates ONE narrow append of a human-gated proposal — the agent process still holds no write-capable database credential | agent proposal client, approval service door |
| `APPROVAL_DATABASE_URL` | **required — fails closed.** The approval service's own credential, and it must authenticate as `switchboard_approval` (migration 014) — the service refuses to start otherwise, naming the role it got. Never derived from `DATABASE_URL`: that role is the migration owner, i.e. the one role able to `grant insert … to switchboard_agent` and retire the published read-only claim rather than defeat it | approval service |
| `APPROVAL_BASE_URL` | **required — fails closed.** Where the agent posts proposals (dev: `http://127.0.0.1:4009`). No default, deliberately: a default makes "the approval service is not running" indistinguishable from "the operator forgot", and the second should refuse rather than retry against localhost forever | agent proposal client |
| `APPROVAL_BIND_HOST` / `APPROVAL_PORT` | `127.0.0.1` / `4009`. Loopback by default — the proposal door's caller is a process on the same host in every deployment shape this repo supports. An operator who really does split the hosts says so explicitly | approval service |
| `APPROVAL_HUMAN_SURFACE` | unset. Set `1` to register the human surface (A0b): `/login`, `/queue`, `/decide` — magic-link login, database-backed sessions (migration 019), CSRF defence. Unset, the service is the agent-door-only deployment, byte for byte. When set, `APPROVAL_SESSION_SECRET` and `APPROVAL_PUBLIC_URL` become **required — fails closed** at boot | approval service |
| `APPROVAL_SESSION_SECRET` | **required when the human surface is on — fails closed** (`ALLOW_DEV_SECRETS=1` substitutes a published dev value, local demo only). Signs the session cookie | approval service |
| `APPROVAL_PUBLIC_URL` | **required when the human surface is on — fails closed.** The origin the APPROVER'S BROWSER uses; every sign-in link is minted under it. Distinct from `APPROVAL_BASE_URL` (where the AGENT posts): on a real deployment the human reaches a TLS hostname while the agent posts loopback | approval service |
| `APPROVAL_COOKIE_INSECURE` | unset. Set `1` ONLY for plain-http local development: renames the session cookie (drops the `__Host-` prefix, which would be a lie without Secure) and clears the Secure attribute, with a loud boot banner. `__Host-`/Secure cookies do not exist over plain http on a LAN address, and express-session refuses to set a Secure cookie on a non-TLS socket — this is the documented dev path, never a deployment setting | approval service |
| *(boot shape)* | Run the human surface through `npx tsx scripts/approval-service.ts` — the composition root that wires the Postmark SMTP transport as the sign-in-link sender (the approver's address must be on `SWITCHBOARD_EMAIL_ALLOWLIST`). `npm run start -w approval` alone refuses the surface unless `ALLOW_DEV_SECRETS=1` prints links to the terminal instead | approval service |
| `PENDING_PROPOSAL_CAP` | `100`. Flood control: the door counts pending proposals for the tenant and refuses at the cap with a loud 429. A compromised agent host holds the door's token and can forge well-formed proposals at volume; the terminal state of that is an approval queue no human can triage, which *disables* the "nothing acts without an identified approver" constraint rather than merely annoying it. Range-checked (integer ≥ 1) at boot, naming the variable — raise it only if a human can actually keep up with the higher number. **After A2 the queue drains itself, and the number counts a different quantity.** The count is now validity-filtered — UNEXPIRED pending rows — so the same numeral denotes something narrower than it did before, and one dead burst no longer 429s the door forever. Pending proposals reach a terminal state **two ways today**. **A decision** (`approved` or `rejected`) requires an `approval.decisions` row of the matching kind naming an approver, written in the same transaction: the trigger refuses a bare `update … set state`, so **manual SQL can no longer reject a proposal** — deliberately, because a rejection is a human decision and must name the human. **Expiry** is automatic: the sweeper moves `pending → expired` at the 72-hour TTL and releases cap budget. (A third path, **amendment**, exists in the SCHEMA ONLY — `supersedes`, `authored_by` and `authored_by_user_id` are there and constrained, but A2 ships no endpoint, function or CLI that creates an amendment, so do not go looking for one. Recorded in KNOWN-ISSUES; `ingest/test/approval-honesty.test.ts` reds if this stops being true in either direction.) If a queue must be drained by hand in an emergency, the correct statement is `update approval.proposals set state = 'expired' where state = 'pending'` as the migration owner — *expired*, not *rejected*, because nobody decided. Since A0b (migration 019) a signed-in approver can also drain it the intended way: magic-link login at `/login`, decisions at `/queue`, every decision row naming her session's user id. A per-action-type rate limit (`PROPOSAL_ACTION_RATE_LIMIT`) sits in front of this cap and is the stronger of the two | approval service door |
| `PROPOSAL_ACTION_RATE_LIMIT` | `20`, over a rolling 60 minutes, **per action type**. A2's runaway backstop, and the stronger of the two volume controls on this door: it bounds how fast a compromised agent host can PRODUCE one kind of action, where the cap only bounds how many can sit in the queue. Counted over every state, because a burst that has already been triaged still happened. 🚨 The number is a JUDGMENT with NO SOURCE — SRE's "2 per 12-hour shift" is a rate derived from handling time, and nobody has measured how long a broker spends on one of these, so the method transfers and the number does not. Range-checked at boot. The usual right answer to hitting it is to NARROW THE ACTION, not to raise the number | approval service door |
| `PROPOSAL_IDEMPOTENCY_FINGERPRINT` | **not a variable — a published contract, recorded here because callers need it.** The proposal door decides whether a reused `idempotency_key` is the SAME ask by comparing exactly three fields: **`action_type`, `payload_hash`, `rationale`** (`approval/src/canonical.ts`, and the same triple the queue uses to collapse duplicate cards). A key reused with a matching fingerprint is an idempotent `200` (or `409` if that ask already reached a terminal state); a key reused with a DIFFERENT fingerprint is a **`422` and nothing is written** — the response names these fields. It is deliberately a declared subset rather than the whole request: a partial fingerprint is fine, an *undocumented* one is not, and `rationale` was previously omitted and silently swallowed. A field added to the proposal grammar must be added here or explicitly exempted — a test pins the list so the choice cannot be made by forgetting | approval service door |
...
| `DBT_SCHEMA` | `public_analytics`. **Reader-side alias, not a deployment knob** (gate-H I10): dbt is never passed this variable — `warehouse/profiles.yml` (`schema: public`) plus `dbt_project.yml` (`+schema: analytics`) build into `public_analytics` unconditionally. Setting it to anything else points every READER at a schema dbt does not fill, and the migrator creates and grants on an empty one. Change the dbt profile too, or leave it alone | agent, report worker, MCP tool, `scripts/verify-identity.ts`, ingest `migrate` — every reader, and dbt not at all |
...
| `DBT_PORT` | `5433` — the port `docker-compose.yml` publishes and `DATABASE_URL` defaults to (gate-H M7: it used to default to 5432, so a manual `dbt build` with the variable unset targeted whatever was on 5432 and, on a developer host with a `switchboard` database there, built a warehouse in the wrong cluster and exited 0) | dbt profile (host port of Postgres) |
...
| `SHEETS_SERVICE_ACCOUNT_KEY_FILE` | unset → **sheet adoption OFF, loudly** (the reconcile loop logs it at boot; linked-sheet rows stop becoming contacts). Set it to the PATH of a Google service-account key file (mode 600, outside the repo) — the value is a file path, NEVER inline JSON, and nothing anywhere logs or echoes the key material. A set-but-unreadable path refuses at boot | CRM reconcile loop (sheet adoption pass), `crm-sheet` CLI |
| `CRM_SHEET_MAX_CHANGES` / `CRM_SHEET_MAX_DRIFT_PCT` | `25` / `20`. The adoption pass's circuit breaker: one pass that would create-or-block more than the first, or whose ref'd rows' name+email diverge by more than the second percent (min sample 5), HALTS before writing and records `breaker_*` in `crm.sheet_reads`. The drift arm exists for the partial-range sort (values scrambled under unmoved refs — invisible to any count). Raise for ONE run when a reorganisation is deliberate; neither arm fires on a sheet's first import | CRM reconcile loop (sheet adoption pass) |

### A send that may have died mid-flight (`executing` with no outcome)

`npm run stuck-executions -w approval` (optionally `-- --minutes 60`) lists every proposal
whose execution *started* and never recorded an outcome, with its age. Needs
`APPROVAL_DATABASE_URL`; it holds `SELECT` and nothing else, deliberately.

🚨 **It diagnoses and does not adjudicate, and neither should you on its output alone.**
Each row is EITHER a send whose executor died OR a send still in flight, and nothing in this
system can tell those apart — only the vendor's delivery semantics can. The approval service
builds no auto-reaper for exactly this reason: a timer that marks a live in-flight send
`failed` is worse than a stuck row, because the human authorised **one** send and a mistaken
`failed` invites a second. Before doing anything with a listed row, check the vendor for that
proposal's `idempotency_key`. There is no supported command that moves these rows; that
contract belongs to the task that owns the sender.
...
- **Malformed payloads:** rows sit in `ingest.quarantine` with reasons and their
  ...
  **Tenant scoping (CLOSE-3):** `--list` and the sweep take an optional
  `[--tenant <uuid>]` and default to **the deployment's own tenant** (`resolveDeploymentTenant()`, i.e. `SWITCHBOARD_TENANT_ID` — see the variable table above), exactly like `gap-ack`,
