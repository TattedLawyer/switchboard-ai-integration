...
//
// 🚨 THE CIRCUIT BREAKER HAS TWO ARMS, and the second exists for a failure the first is
// blind to. COUNT: a single pass that would adopt or block more than a threshold halts —
// a wrong sheet, a mass deletion, a half-loaded snapshot. DRIFT: a PARTIAL-RANGE SORT (she
// selects some columns and sorts — a one-click UI mistake) scrambles VALUES against rows
// while metadata stays put: no ref vanishes, no ref moves, the count arm sees nothing, and
// every row now binds one person's name to another's number. So when more than K% of
// ref'd rows change name+email in one pass, the pass halts and says so. Both arms halt
// BEFORE any write. Neither arm runs on a sheet's FIRST import (there is no stored state
// to corrupt, and importing every row is the point of linking).
//
...
/** Percentage of ref'd rows whose name+email may change in one pass. Env:
 *  CRM_SHEET_MAX_DRIFT_PCT. */
export const DEFAULT_MAX_DRIFT_PCT = 20;
/** The drift arm needs a sample: below this many matched refs, one legitimate edit is a
 *  huge percentage, so the arm stays quiet. */
 ...
 * is the failure mode this repo names as its worst.
 */
export async function runSheetAdoption(
  ...
  const driftMinSample = deps.thresholds?.driftMinSample ?? DRIFT_MIN_SAMPLE;
  ...
  const dataRows: SheetContactRow[] = tab.rows
    ...
    .filter((r) => r.ref !== null || r.cells.some((c) => c.trim() !== ""))
  ...
  const divergent = matched.filter((r) => {
    const c = byRef.get(r.ref as string) as BoundContact;
    return (
      norm(c.display_name) !== norm(r.fields.displayName) ||
      norm(c.email_address) !== norm(r.fields.emailAddress)
    );
  });

  // ── 3. The breaker — both arms, BEFORE any write, never on a first import. ───────────
  if (!firstImport) {
  ...
  // re-baseline, months of legitimate edits would eventually trip the drift arm).
  for (const row of matched) {
    ...
    try {
      if (!c.active) {
        ...
        report.reactivated++;
      } else if (divergent.includes(row)) {
        await admin.query(
          `update crm.contacts
  ...
  // ── 7. Returned rows: close the block, restart the clock. ────────────────────────────
