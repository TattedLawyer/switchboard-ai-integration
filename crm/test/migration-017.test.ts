// Email spike / Task 1 pins — migration `017_email_disposition.sql`.
//
// 🚨 RED BY MEASUREMENT, NOT BY ABSENCE. Before 017 exists, the first pin measures `23514`
// (check violation) rather than "module not found". Every write below goes through the
// `crm` pool — `switchboard_crm`, the role the executor actually connects as — because a
// pin that widens a CHECK while the executing role lacks the privilege to write the new
// value is a pin over a universe the shipped code cannot reach.
//
...
describe("Task 1: 'sent' is a representable disposition", () => {
  // mutation: delete the `'sent'` literal from 017's rebuilt CHECK -> red. RUN ✅ 2026-08-12
  //   Observed: `Tests  1 failed | 2 passed (3)`
  //     AssertionError: expected '23514' to be 'NO-ERROR'
  //   i.e. `switchboard_crm` could not record the submission at all — the exact state the
  //   repo was in before 017, measured rather than assumed.
  it("lets switchboard_crm record a submitted email as 'sent'", async () => {
    ...
    const code = await sqlstate(() =>
      crm.query(`update crm.touches set disposition = 'sent' where id = $1`, [touchId]),
    );
    ...
    expect(r.rows[0].disposition).toBe("sent");
  });
  ...
  // The CHECK was WIDENED BY EXACTLY ONE MEMBER, not dropped. Each of 016's nine call
  // outcomes must still be writable.
  it("keeps all nine call dispositions writable", async () => {
