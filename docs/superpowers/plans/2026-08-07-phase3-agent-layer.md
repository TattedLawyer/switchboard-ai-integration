...
🚨 **CORRECTION from ADR-1, THEN SUPERSEDED BY A1 (2026-08-07).** Rev 2 claimed the host process is "already the full-privilege app role." **It is not** — `agent/src/host/run-report.ts:8` builds the host's only pool from `agentConnectionString()`, which resolves to the read-only `switchboard_agent`. That correction stands. The consequence it derived — *"A1 therefore inherits unpriced work: a second writable pool in the host"* — **is struck.** ~~a second writable pool in the host~~ Do not add one. A1 established, after two research passes and an adversarial review, that the property "the agent process holds no write-capable credential" was one env var away rather than held, and that an in-process pool would destroy it permanently and irrecoverably. **The writer lives in the client-facing approval service A0b already requires** (marginal deployment cost: zero processes), connecting as a dedicated non-owner `switchboard_approval` role; the agent hands it a validated object across an authenticated, loopback-bound door and imports no `pg` at all. The containment rule survives in a stronger form: there is no writer in the process to keep away from the tool surface. See the Amendment section of `docs/adr/agent-writer-boundary.md`.
