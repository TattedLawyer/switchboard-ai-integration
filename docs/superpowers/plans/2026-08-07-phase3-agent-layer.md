...
🔗 **Synergy to exploit:** C5 builds transactional email with her domain authenticated. **Magic-link authentication rides that same channel** — no password storage, no reset flow, and one fewer credential for a non-technical user to lose. Sequence A0b's implementation after C5's channel exists, or stub the send behind the same interface.
...
**A1 — Action and intent objects, and the tool-call boundary.** Typed, immutable proposal: what, to whom, with what payload, derived from which records, with an idempotency key.
...
**D1 — Vendor spike, IN PARALLEL with Track A** (procurement, not build). **Required outputs, not "choose a bridge":** confirm Filipino on the **Live API specifically** (confirmed for speech generation; the Live docs did not enumerate languages), test **Taglish** code-switching quality, obtain the **LiveKit outbound SIP surcharge** and the **Telnyx PH termination rate** (both explicitly unverified in the vendor research), and choose the bridge. Leading candidate LiveKit Cloud + BYO carrier (only maintained Gemini Live integration; carrier-agnostic). Twilio direct is the zero-risk fallback at effectively equal cost — carrier minutes (~$0.29/min PH mobile) dominate platform fees (~$0.0044/min) ~65×. **Ruled out:** Vapi and Retell (their "bring your own LLM" is a *text* endpoint, demoting Gemini Live to a text model behind their STT/TTS), Bland (no BYO), Synthflow (~$30k/yr).
