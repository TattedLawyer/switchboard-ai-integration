...
- **Contract extension** (`ingest/src/numeric-contract.ts` → string rules): `{ pattern, required }` for text fields so garbage currency cells quarantine with named reasons at the door (`^[A-Z]{3}$`); free-hand dates ship as passthrough strings — `close_date` has no rule yet, and date rules are registered follow-up work. Wave 5's currency-at-door lands here; the count-presence trade is documented from the register; staging's regexp stays as doorless-row containment.
