...
- A worker that generates the Monday revenue-risk report — with a timeout and
  ...
  cost logging.
- **CI:** the `ci` workflow runs on every push — typecheck, all 1024 tests, the
...
- 1024 automated tests, green in CI and locally — including a seeded
...
| Suite | `npm test` + dbt | 1024 tests green across nine workspaces (incl. 6 seeded fast-check properties); 101 dbt build steps (15 models, 3 seeds, 83 data tests) — `PASS=100 WARN=1 ERROR=0`, the one warn deliberate and mechanically pinned (see below) |
