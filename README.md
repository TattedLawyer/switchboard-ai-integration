...
- **CI:** the `ci` workflow runs on every push — typecheck, all 1991 tests, the
...
- 1991 automated tests, green in CI and locally — including a seeded
...
| Suite | `npm test` + dbt | 1991 tests green across twelve workspaces (incl. 6 seeded fast-check properties); 101 dbt build steps (15 models, 3 seeds, 83 data tests) — `PASS=100 WARN=1 ERROR=0`, the one warn deliberate and mechanically pinned (see below) |
