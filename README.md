...
- A worker that generates the Monday revenue-risk report — with a timeout and
  ...
  cost logging.
- **CI:** the `ci` workflow runs on every push — typecheck, all 924 tests, the
...
- 924 automated tests, green in CI and locally — including a seeded
...
| Suite | `npm test` + dbt | 924 tests green across nine workspaces (incl. 6 seeded fast-check properties); 98 dbt build steps (15 models, 2 seeds, 81 data tests) — `PASS=97 WARN=1 ERROR=0`, the one warn deliberate and mechanically pinned (see below) |
